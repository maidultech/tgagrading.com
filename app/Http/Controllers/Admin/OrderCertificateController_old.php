...
class OrderCertificateController extends Controller
{
    ...
    public function update( Request $request, Order $order )
    {
        ...
        $request->validate([
            ...
            'centering.*.*' => 'required_without:cert_no_grade.*.*|numeric|max:10.00',
            'corners.*.*' => 'required_without:cert_no_grade.*.*|numeric|max:10.00',
            'edges.*.*' => 'required_without:cert_no_grade.*.*|numeric|max:10.00',
            'surface.*.*' => 'required_without:cert_no_grade.*.*|numeric|max:10.00',
       ...
       try {
        foreach ( $request->card as $details_id => $details ) {
            ...
            foreach ( $details as $key => $card_number ) {
                ...
                $cardCheck = OrderCard::find($key);

                if ( !is_null($cardCheck) ) {

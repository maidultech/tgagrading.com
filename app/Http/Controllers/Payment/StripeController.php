...
class StripeController extends Controller
{
    ...
    function makePayment( Request $request, $trx_id, $price = null)
    {
        ...
        try {
            ...
            if($price == null)
            {
                ...
                {
                    $gst = max(0,$order->gst_tax*($price - ($order->coupon_discount ?? 0) - $userWalletBalance)/100);
                    $pst = max(0,$order->pst_tax*($price - ($order->coupon_discount ?? 0) - $userWalletBalance)/100);
                } else {
                    $gst = max(0,$order->gst_tax*($price - ($order->coupon_discount ?? 0))/100);
                    $pst = max(0,$order->pst_tax*($price - ($order->coupon_discount ?? 0))/100);
                }
                ...
                if($total_tax > 0) {
                    $product = $stripe->products->create([
                        'name' => 'Tax',
                        'description' => "GST ({$order->gst_tax}%) : $" . number_format($gst, 2) . ",\n\nPST ({$order->pst_tax}%) : $" . number_format($pst, 2),
                    ]);
            
                    $price = $stripe->prices->create([
                        'currency' => 'cad',
                        'unit_amount' => round(($gst + $pst) * 100),
                        'product' => $product->id,
                    ]);
    ...
    function makeOrderPayment( Request $request, $trx_id)
    {
        ...
        try {
            ...
            } elseif($order->is_redeemed == 1 && $order->extra_cards <= 0) { 
                $total_price += ($order->net_unit_price*0);
            } else {
            ...
            foreach ($order->additionalCosts as $cost) {
                ...
                // Create Stripe product for each additional cost
                $AdditionalCharge = $stripe->products->create([
                    'name' => $cost->details, 
                ]);

                // Create Stripe price for each additional cost
                $AdditionalChargeCost = $stripe->prices->create([
                    'currency' => $currency,
                    'unit_amount' => $cost->price * 100,
                    'product' => $AdditionalCharge->id,
                ]);

                // Add this price to the price list
                $priceList[] = [
                    'price' => $AdditionalChargeCost->id,
                    'quantity' => 1,
                ];
            ...
            if ($used_wallet == 1 && $userWalletBalance > 0)
            {
                $gst = max(0,$order->gst_tax*($total_price - ($order->coupon_discount ?? 0) - $userWalletBalance + $order->shipping_charge)/100);
                $pst = max(0,$order->pst_tax*($total_price - ($order->coupon_discount ?? 0) - $userWalletBalance)/100);
            } else {
                $gst = max(0,$order->gst_tax*($total_price - ($order->coupon_discount ?? 0) + $order->shipping_charge)/100);
                $pst = max(0,$order->pst_tax*($total_price - ($order->coupon_discount ?? 0))/100);
            }
            $total_tax = $gst + $pst;
            ...
            if($total_tax > 0) {    
                ...
                $product = $stripe->products->create([
                    'name' => 'Tax',
                    'description' => "GST ({$order->gst_tax}%): ".number_format($gst,2).", PST ({$order->pst_tax}%): ".number_format($pst,2),
                ]);
            ...
            //         'currency' => $currency,
            //     ],
            //     'display_name' => 'Shipping Charge ('.$order->shipping_method == 'ups' ? 'UPS Standard' : $order->shipping_method.')',
            //     'type' => 'fixed_amount'
            // ]);

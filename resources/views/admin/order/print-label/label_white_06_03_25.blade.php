...
<body>
    ...
    <div class="mini_card position-relative overflow-hidden">
        <div class="row align-items-center justify-content-center w-100 d-flex"
            ...
            <div class="col-5">
                <div class="watermark_logo position-relative" style="{{ $cert->details?->notes || $cert->notes ? '' : 'top: -5px;' }}">
                    ...
                    @php
                        ...
                        $tga_logo_left = $grading_condition ? (strpos((string) $cert->surface, '.5') !== false  
                                        || strpos((string) $cert->corners, '.5') !== false 
                                        ? 'left: 39.5%;' 
                                        : 'left: 39%;') 
                                        : '';
                    ...
                    <div class="" style="padding: 0px 5px;">
                        <div class="card_info" style="width:50%; float:left; position: relative;{{ $cert->details?->notes || $cert->notes ? 'bottom:-14px' : 'bottom:-1.43rem' }}; {{getTitleStyle(($cert->details->card ?? $cert->card), 'position')}}">
                            <div class="gradings">
                        ...
                        <div class="card_info" style="width:50%; float:right; position: relative; text-align: right; left: {{ $grading_condition ? '23px' : '22px' }}; {{ $cert->details?->notes || $cert->notes ? 'bottom:-14px' : 'bottom:-1.43rem' }}; {{getTitleStyle(($cert->details->card ?? $cert->card), 'position')}}">
                            <div class="gradings">
                ...
                <div class="" style="text-align: right !important;">
                    <div class="logo_qr">
                        {!! QrCode::size(60)->style('round')->color(0, 0, 0)->backgroundColor(255, 255, 255, 0)->generate(
                            route('frontend.certification', ['number' => $isManual ? $cert->qr_link : $cert->card_number,])) 
                        !!}
                    </div>
                </div>
            </div>

...
class CanadaPostService extends ServiceProvider
{
    /**
     * Service's name.
     */
    ...
    public function ship(ShipFrom $shipFrom, ShipTo $shipTo, BoxCollection $boxes, ServiceProviderService $serviceProviderService, ServiceProviderShipmentCustomsValue|null $serviceProviderShipmentCustomsValue = null, $customData = []): ServiceProviderShipmentCollection
    {
        ...
        foreach ($boxes as $box) {
            ...
            if (!empty($shipTo->email())) {
                $body['delivery-spec']['notification'] = [
                    'email' => $shipTo->email(),
                    'on-shipment' => true,
                    'on-exception' => true,
                    'on-delivery' => true,
            ...
            if ($serviceProviderShipmentCustomsValue !== null) {
                $body['delivery-spec']['customs'] = array_merge(
                    [
                        'currency' => $serviceProviderShipmentCustomsValue->currency(),
                    ], $serviceProviderShipmentCustomsValue->customData()
                );
            }
    ...
    function track(Order $order){
        ...
        $url = $this->credentials->test() ? 'https://ct.soa-gw.canadapost.ca' : 'https://soa-gw.canadapost.ca';
        
        // $pin = $order->shipping_api_response['shipment-info']['tracking-pin'];

        $url = "{$url}/vis/track/pin/{$order->admin_tracking_id}/detail";

        $response = Http::withHeaders([
            'Accept' => 'application/vnd.cpc.track-v2+xml',
            'Accept-language' => 'en-CA', // or 'fr-CA' for French
        ])->withBasicAuth(
            $this->credentials->username(), 
            $this->credentials->password()
        )->get($url);
        if ($response->successful()) {
           return XML2Array::createArray($response->body());
        } else {
            // return XML2Array::createArray(File::get(public_path('/assets/tracking.xml')));
            return false;
        }

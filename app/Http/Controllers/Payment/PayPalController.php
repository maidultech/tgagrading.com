    ...
    public function processTransaction( Request $request , $trx_id, $price = null)
    {
        try {
            ...
            if($price == null)
            {
                ...
                $gst = $order->gst_tax*($price - ($order->coupon_discount ?? 0) - $walletDiscount)/100;
                $pst = $order->pst_tax*($price - ($order->coupon_discount ?? 0) - $walletDiscount)/100;
                $total_tax = $gst + $pst;
                ...
                array_push($priceList, [
                    "amount" => [
                        ...
                        "breakdown" => [
                                    "item_total" => [
                                        "currency_code" => 'CAD',
                                        "value" => number_format($totalItemValue, 2, '.', ''),
                                    ],
                                    "discount" => [
                                        "currency_code" => 'CAD',
                                        "value" => number_format($totalDiscountAmount ?? 0, 2, '.', ''),
                                    ],
                                ],
                    ],
    ...
    public function processOrderTransaction( Request $request , $trx_id)
    {
        try {
            ...
            if($order->is_redeemed == 1) {
                ...
                if($order->extra_cards > 0) {
                    $cardCount = $order->extra_cards;
                } elseif($order->extra_cards < 0) {
                    $cardCount = 0;
                }
                ...
                if ($order->coupon_id) {
                    $perItemDiscount = $order->coupon_discount;
                }
                if ($used_wallet == 1 && $userWalletBalance > 0) {
                    $perItemWalletDiscount = $userWalletBalance;
                }
                ...
                foreach ($order->details as $key => $item) {
                    $cardCount = OrderCard::where('order_details_id', $item->id)->count();
            ...
            $total_price += $order->additionalCosts->sum('price');
            $gst = $order->gst_tax*($total_price - ($order->coupon_discount ?? 0) - $walletDiscount + $order->shipping_charge)/100;
            $pst = $order->pst_tax*($total_price - ($order->coupon_discount ?? 0) - $walletDiscount)/100;
            $total_tax = $gst + $pst;

            $totalValue = $totalValue + $total_tax + $order->shipping_charge + $order->additionalCosts->sum('price');
            $totalItemValue = $totalItemValue + $total_tax + $order->shipping_charge + $order->additionalCosts->sum('price');

@extends('frontend.layouts.app')

@section('title')
    {{ 'Checkout' }}
@endsection



@push('style')

@endpush

@section('content')
    <!-- ======================= breadcrumb start  ============================ -->
    @section('breadcrumb')
        <li class="breadcrumb-item">{{ __('Checkout') }}</li>
    @endsection
    <!-- ======================= breadcrumb end  ============================ -->
    <!-- ======================= checkout start  ============================ -->
    <div class="checkout-sec pb-5">
        <div class="container">
            <div class="mb-3 mb-lg-5">
                <div class="checkout_steps text-center">
                    @include('checkout.steps')
                </div>
            </div>
            <form action="{{ route('checkout.item.type.store',$plan->id) }}" method="post">
                @csrf
                <div class="row gy-4 gy-lg-0">
                    <div class="col-lg-4 order-lg-2">
                        <div class="checkout_sidebar rounded  rounded-4 border-light border-light-subtle card border-1 p-4 sticky-top"
                             style="top:1rem;">
                            <div class="heading mb-4">
                                <h4>Summary</h4>
                            </div>
                            <div class="sidebar_box">
                                <table class="table table-bordered">
                                    <tr>
                                        <td>Item Type</td>
                                        <td class="text-end">Trading Cards</td>
                                    </tr>
                                    {{-- @if($total_cards > 0)
                                        @php
                                            $subscription = $user->subscriptions->where('year_start', '<', now())->where('year_end', '>', now())->first();
                                            $available_card_limit = $subscription ? $subscription->subscription_card_peryear - $subscription->order_card_peryear : 0;
                                        @endphp
                                        @if($plan->type == 'subscription')
                                        @elseif($user->is_subscriber && $user->subscription_start < now() && $user->subscription_end > now() && $available_card_limit > 0)
                                        @else
                                        <tr>
                                            <td>Items x <span
                                                    class="item-count">{{ session('checkout.items') ? $total_cards : 0 }}</span>
                                            </td>
                                            <td class="text-end">$ <span
                                                    class="totalprice">{{ number_format($total_cards * session('checkout.total_unit_price'),2,'.','') }}</span>
                                            </td>
                                        </tr>
                                        @endif
                                    @endif --}}
                                    {{-- <tr>
                                        <td>Submission Type</td>
                                        <td class="text-end">Grading</td>
                                    </tr> --}}
                                </table>
                                <button class="btn w-100 btn-primary py-3 mt-4">
                                    Select & Continue
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-8 order-lg-1">
                        <div class="checkout_wrapper px-xl-5">
                            <div class="heading mb-4">
                                <h4>Select Item Type</h4>
                                <p>Select the item type that you are submitting to TGA</p>
                            </div>
                            <div class="item_list">
                                @foreach (config('static_array.item_type') as $key => $item)
                                <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" value="{{ $loop->iteration }}" @checked($loop->index==0) type="radio" name="item_type" id="item_{{ $key }}">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_{{ $key }}">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-id-card"></i>
                                            </div>
                                            <div class="content-width">
                                                <div class="name fw-semibold">{{ $item }}</div>
                                                <div class="info">Standard size, Pokémon, thicker cards up to 180pt, and mini size sports cards</div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                @endforeach
                                <!-- <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" type="radio" name="type" id="item_2">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_2">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-id-card-clip"></i>
                                            </div>
                                            <div class="">
                                                <div class="name fw-semibold">Oversized Trading Cards</div>
                                                <div class="info">Supersized, T3 & Jumbo cards</div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" type="radio" name="type" id="item_3">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_3">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-coins"></i>
                                            </div>
                                            <div class="">
                                                <div class="name fw-semibold">Coins & Pins</div>
                                                <div class="info">Hobby-issued coins and pins</div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" type="radio" name="type" id="item_4">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_4">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-ticket"></i>
                                            </div>
                                            <div class="">
                                                <div class="name fw-semibold">Tickets</div>
                                                <div class="info">Full, stub, pass, commemorative & badge tickets</div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" type="radio" name="type" id="item_5">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_5">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-star"></i>
                                            </div>
                                            <div class="">
                                                <div class="name fw-semibold">Funko Pop!</div>
                                                <div class="info">Standard size Funko Pop!</div>
                                            </div>
                                        </div>
                                    </label>
                                </div>

                                <div class="form-check mb-2 p-0">
                                    <input class="form-check-input d-none" type="radio" name="type" id="item_6">
                                    <label class="form-check-label p-3 rounded-4 border w-100" for="item_6">
                                        <div class="d-flex align-items-center">
                                            <div class="icon me-3">
                                                <i class="fa fa-box"></i>
                                            </div>
                                            <div class="">
                                                <div class="name fw-semibold">Funko Pop! FASC</div>
                                                <div class="info">From a sealed Funko Pop! case (27, 36 quantity)</div>
                                            </div>
                                        </div>
                                    </label>
                                </div> -->

                            </div>
                        </div>
                    </div>
                </div>
            </form>
        </div>
    </div>
    <!--======================= checkout end ============================ -->

@endsection

@push('script')
@endpush

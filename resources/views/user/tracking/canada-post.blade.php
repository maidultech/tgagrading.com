    ...
    <div class="account_seciton pb-5 pt-3">
        <div class="container">
            ...
            <div class="row gy-4 gy-lg-0 mt-5">
                ...
                <div class="row">
                    <div class="col-12">
                        @if (isset($trackingDetails['is-testing']) and $trackingDetails['is-testing']["@value"] == 'true')
                            <div class="alert alert-warning">
                                <strong>Testing Mode:</strong> This is a testing mode. The tracking information may not be accurate.
                            </div>
                        @endif
                    </div>
                </div>
                <div class="row mt-4">
                    <div class="col-md-6">
                        <p><strong>Tracking Number:</strong> {{$trackingDetails['pin']}}</p>
                        <p><strong>Destination:</strong> 
                            ...
                            {{$trackingDetails['destination-postal-id']}}
                        </p>
                        {{-- <p><strong>Delivery Status:</strong> <span title="{{ $trackingStatus }}">{{$trackingStatus}}</span></p> --}}

                        {{-- <p><strong>Expected Delivery:</strong> @if(isset($trackingDetails['expected-delivery-date'])){{ \Carbon\Carbon::parse($trackingDetails['expected-delivery-date'])->format('F j, Y') }}@endif</p> --}}
                        @if (isset($trackingDetails['changed-expected-delivery-reason']) && $trackingDetails['changed-expected-delivery-reason'])
                            <p class="text-danger"><strong>Changed Expected Delivery Reason:</strong> {{$trackingDetails['changed-expected-delivery-reason']}}</p>                            
                        @endif
                        <p><strong>Carrier Name:</strong> {{str($order->shipping_method)->headline()}}</p>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="mt-4">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @php
                            $occurrences = $trackingDetails['significant-events']['occurrence'] ?? [];
                                if (isset($occurrences['event-date'])) {
                            ...
                            @foreach ($occurrences as $event)
                                <tr>
                                    <td>
                                        {{ \Carbon\Carbon::parse($event['event-date'])->format('F d, Y') ?? 'N/A' }}
                                    </td>
                                    <td>
                                        {{ \Carbon\Carbon::parse($event['event-time'])->format('h:i A') ?? 'N/A' }} {{ $event['event-time-zone'] ?? '' }}
                                    </td>
                                    <td>
                                        {{ $event['event-site'] ?? '' }}
                                    </td>

...
class SettingsController extends Controller
{
    ...
    public function __construct(Setting $websetting)
    {
    ...
    {
        // dd($request->all()) ;
        // if (is_null($this->user) || !$this->user->can('admin.settings.edit')) {
        //     abort(403, 'Sorry !! You are Unauthorized.');
        // }
        // dd($request->all()); 
        ...
        try {
            ...
            // $setting->gst_tax = $request->gst_tax;
            // $setting->pst_tax = $request->pst_tax;
            $setting->google_recaptcha = $request->google_recaptcha;
            $setting->recaptcha_site_key = $request->recaptcha_site_key;
            $setting->recaptcha_site_secret = $request->recaptcha_site_secret;
            $setting->zendesk_subdomain = $request->zendesk_subdomain;
            $setting->zendesk_email = $request->zendesk_email;
            $setting->zendesk_token = $request->zendesk_token;

            $setting->canadapost_username = $request->canadapost_username;
            $setting->canadapost_password = $request->canadapost_password;
            $setting->canadapost_customer_number = $request->canadapost_customer_number;
            $setting->canadapost_contact_id = $request->canadapost_contact_id;
            $setting->canadapost_mode = $request->canadapost_mode;
            $setting->ups_client_id = $request->ups_client_id;
            $setting->ups_client_secret = $request->ups_client_secret;
            $setting->ups_user_id = $request->ups_user_id;
            $setting->ups_mode = $request->ups_mode;
            $setting->shipping_cost_maximization = $request->shipping_cost_maximization;
            $setting->insurance_cost = $request->insurance_cost;
            $setting->min_bulk_grading_cost = $request->min_bulk_grading_cost;
            $setting->max_bulk_grading_cost = $request->max_bulk_grading_cost;
            ...
            if ( $request->hasFile('site_logo') ) {
                ...
                // decreaseImageOpacity(public_path($setting->site_logo),.3);
            ...
            if ($request->hasFile('order_confirmation_image')) {
                $order_confirmation_image = $request->file('order_confirmation_image');
                $base_name = preg_replace('/\..+$/', '', $order_confirmation_image->getClientOriginalName());
                $base_name = explode(' ', $base_name);
                $base_name = implode('-', $base_name);
                $base_name = Str::lower($base_name);
                $image_name = $base_name . "-" . uniqid() . "." . $order_confirmation_image->getClientOriginalExtension();
                $file_path = '/assets/uploads/logo';
                $order_confirmation_image->move(public_path($file_path), $image_name);
                $setting->order_confirmation_image = $file_path . '/' . $image_name;
            ...
            if ( $request->hasFile('flip_preview') ) {
                $flip_preview = $request->file('flip_preview');
                $base_name = preg_replace('/\..+$/', '', $flip_preview->getClientOriginalName());
                $base_name = explode(' ', $base_name);
                $base_name = implode('-', $base_name);
                $base_name = Str::lower($base_name);
                $image_name = $base_name . "-" . uniqid() . "." . $flip_preview->getClientOriginalExtension();
                $file_path = '/assets/uploads/preview';
                $flip_preview->move(public_path($file_path), $image_name);
                $setting->flip_preview = $file_path . '/' . $image_name;
            }
            $setting->update();
            ...
            if ( $request->filled('stripe_enable') ) {
                DB::table('config')->where('config_key', 'stripe_enable')->update([
                    'config_value' => $request->stripe_enable,
                ]);
            }
            if ( $request->filled('paypal_enable') ) {
                DB::table('config')->where('config_key', 'paypal_enable')->update([
                    'config_value' => $request->paypal_enable,
                ]);
            }
    ...
    {
        $certificate_verification = CertificateVerification::first();
        return view('admin.certificate', compact('certificate_verification'));
    }

    public function certificateStore(Request $request)
    {
        try {
            $certificateVerification = CertificateVerification::first();
            ...
            if ($request->hasFile('image')) {
                ...
                $certificateVerification->image = uploadGeneralImage($file, 'certificate_verification', $certificateVerification->image);
            }
